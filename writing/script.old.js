...
function handleProgrammeStart() {
  ...
  if (!questionsData || !questionsData[programmeKey]) {
    showNotification(
      "No writing questions found for the selected programme.",
      "error"
    );
    return;
  }
...
function renderScrambleQuestion(question, index) {
  ...
  header.innerHTML = `<span>Sentence ${
    index + 1
  }</span><span>Tap words to build the sentence.</span>`;
  ...
  footer.textContent =
    "Tip: Click a word to add or remove it. Build the sentence in the correct order.";
...
function handleWordChipToggle(questionId, tokenId) {
  ...
  const chip = container.querySelector(
    `.word-chip[data-token-id="${tokenId}"]`
  );
  if (!chip) {
    return;
  }
...
function getTotalQuestionCount() {
  const task1Count =
    activeTasks.task1 && activeTasks.task1.question
      ? Math.max(activeTasks.task1.question.length - 1, 0)
      : 0;
  const task2Count =
    activeTasks.task2 && activeTasks.task2.question
...
function calculateScore() {
  ...
  if (activeTasks.task1 && activeTasks.task1.question) {
    activeTasks.task1.question.slice(1).forEach((entry) => {
      ...
      const isCorrect = expected.some(
        (answer) => normalizeText(answer) === normalizedUser
      );
      if (isCorrect) {
        score += 1;
      }
  ...
  if (activeTasks.task2 && activeTasks.task2.question) {
    activeTasks.task2.question.forEach((entry) => {
      ...
      const normalizedUser = normalizeText(userValue);
      const possibleAnswers = Array.isArray(entry.answers)
        ? entry.answers
        : entry.answer
        ? [entry.answer]
        : [];
  ...
  if (activeTasks.task1 && activeTasks.task1.question) {
    activeTasks.task1.question.slice(1).forEach((entry) => {
      ...
      if (userValue !== "N/A" && entry.answers && entry.answers.length) {
        const normalizedUser = normalizeText(userValue);
        const isCorrect = entry.answers.some(
          (answer) => normalizeText(answer) === normalizedUser
        );
        outcome = isCorrect ? "✓" : "✗";
      }
