...
function handleProgrammeStart() {
  ...
  if (!programmeSets) {
    showNotification(
      "No vocabulary questions found for the selected programme.",
      "error"
    );
    return;
  }
  ...
  selectedQuestions =
    JSON.parse(JSON.stringify(programmeSets[selectedSetKey])) || [];
...
function restoreFromSavedState(savedState) {
  ...
  selectedSetKey =
    savedState.setKey && programmeSets[savedState.setKey]
      ? savedState.setKey
      : Object.keys(programmeSets)[0];

  selectedQuestions = JSON.parse(
    JSON.stringify(programmeSets[selectedSetKey] || [])
  );
